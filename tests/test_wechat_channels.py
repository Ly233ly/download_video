...
class CaptureServiceLifecycleTests(unittest.TestCase):
    ...
    def test_start_keeps_a_pac_vpn_as_the_capture_upstream(self) -> None:
        original = ProxySnapshot(
            {
                "ProxyEnable": RegistryValue(True, 0, 4),
                "ProxyServer": RegistryValue(False),
                "AutoConfigURL": RegistryValue(
                    True,
                    "http://127.0.0.1:10809/proxy.pac",
                    1,
                ),
                "AutoDetect": RegistryValue(False),
            }
        )
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            resolver = Mock()
            resolver.resolve.return_value = ("127.0.0.1", 10809)
            proxy = Mock()
            proxy.address = ("127.0.0.1", 20230)
            proxy_factory = Mock(return_value=proxy)
            service = WechatChannelsCaptureService(
            ...
            try:
                with patch(
                    "idm_eagle_bridge.wechat_channels.upstream_auto_proxy_resolver",
                ...
                self.assertIs(
                    proxy_factory.call_args.kwargs["upstream_resolver"],
                    resolver,
                )
                self.assertEqual(
    ...
    def test_manual_media_proxy_is_capture_upstream_when_wininet_is_direct(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            coordinator = MediaCoordinator(Database(root / "bridge.db"))
            coordinator.network_proxy.configure("manual", "127.0.0.1:7890")
            ...
            try:
                ...
                self.assertEqual(
                    proxy_factory.call_args.kwargs["upstream_proxy"],
                    ("127.0.0.1", 7890),
                )
                service.stop()
                self.assertEqual(backend.current, direct)
            finally:
                service.close()
                coordinator.close()

    def test_auto_mode_uses_a_saved_media_proxy_only_while_it_is_live(self) -> None:
        direct = ProxySnapshot(
            {
                "ProxyEnable": RegistryValue(True, 0, 4),
                "ProxyServer": RegistryValue(False),
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            coordinator = MediaCoordinator(Database(root / "bridge.db"))
            coordinator.network_proxy.configure("auto", "127.0.0.1:7890")
            proxy = Mock()
            proxy.address = ("127.0.0.1", 20230)
            proxy_factory = Mock(return_value=proxy)
            service = WechatChannelsCaptureService(
            ...
            service.proxy_lease.backend = FakeRegistryBackend(direct)
            try:
                with patch(
                    "idm_eagle_bridge.wechat_channels.proxy_endpoint_reachable",
                    return_value=True,
    ...
    def test_auto_mode_ignores_a_saved_media_proxy_while_it_is_offline(self) -> None:
        direct = ProxySnapshot(
            {
                "ProxyEnable": RegistryValue(True, 0, 4),
                "ProxyServer": RegistryValue(False),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertIsNone(proxy_factory.call_args.kwargs["upstream_proxy"])
            finally:
                service.close()
                coordinator.close()

    def test_unreachable_manual_media_proxy_points_to_network_settings(self) -> None:
        direct = ProxySnapshot(
            {
                "ProxyEnable": RegistryValue(True, 0, 4),
                "ProxyServer": RegistryValue(False),
            ...
            try:
                with patch(
                    ...
                    with self.assertRaisesRegex(
                        WechatChannelsError,
                        "留底手动代理 127.0.0.1:7890.*设置 > 网络代理.*自动",
                    ):
                        service.start(
                            configure_system_proxy=False,
    ...
    def test_unusable_pac_fails_before_the_system_proxy_is_replaced(self) -> None:
        original = ProxySnapshot(
            {
                "ProxyEnable": RegistryValue(True, 0, 4),
                "AutoConfigURL": RegistryValue(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            backend = FakeRegistryBackend(original)
            resolver = Mock()
            resolver.resolve.side_effect = CaptureProxyError("PAC unavailable")
            proxy_factory = Mock()
            service = WechatChannelsCaptureService(
                coordinator,
                root=root / "wechat-channels",
...
class CertificateTests(unittest.TestCase):
    ...
    def test_uninstall_bounds_a_stuck_certificate_store_delete(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            calls: list[tuple[list[str], int | None]] = []

            def runner(arguments: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
                calls.append((arguments, kwargs.get("timeout")))
                if "-delstore" in arguments:
                    raise subprocess.TimeoutExpired(arguments, kwargs.get("timeout"))
                return subprocess.CompletedProcess(arguments, 0, "", "")

            authority = WechatCertificateAuthority(
                directory,
                runner=runner,
            )
            authority.ensure()

            with self.assertRaisesRegex(RuntimeError, "移除视频号本机捕获证书超时"):
                authority.uninstall()
            self.assertTrue(any("-delstore" in arguments for arguments, _timeout in calls))
            self.assertTrue(all(timeout == 15 for _arguments, timeout in calls))

    def test_uninstall_reports_certificate_store_failure(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            def runner(arguments: list[str], **_kwargs: object) -> subprocess.CompletedProcess[str]:
                return subprocess.CompletedProcess(
                    arguments,
                    1 if "-delstore" in arguments else 0,
                    "",
                    "certificate store unavailable",
                )
            ...
            with self.assertRaisesRegex(RuntimeError, "无法移除视频号本机捕获证书"):
                authority.uninstall()
...
class CandidateRegistryTests(unittest.TestCase):
    ...
    def test_candidate_list_orders_current_video_first(self) -> None:
        with patch(
    ...
    def test_all_candidates_are_listed_and_activation_orders_current_first(self) -> None:
    ...
    def test_plan_uses_selected_variant_and_keeps_secret_only_in_runtime_payload(self) -> None:
        ...
        # 捕获来源与媒体主机同属腾讯域（qq.com），Cookie 随下载请求发送以满足 CDN 清晰度校验。
        self.assertEqual(
            payload["streams"][0]["headers"].get("Cookie"),
            "session=must-not-cross-domains",
        )
    ...
    def test_quality_spec_becomes_a_selectable_delivery_variant(self) -> None:
        ...
        self.assertEqual(candidate_view["variants"][0]["deliverySpec"], "hd")
        self.assertFalse(candidate_view["variants"][0]["isOriginal"])
        self.assertEqual(candidate_view["variants"][-1]["deliverySpec"], "")
        ...
        self.assertEqual(payload["streams"][0]["label"], "1080p · hd")
        self.assertEqual(payload["streams"][0]["name"], spec_variant.variant_id)

    def test_implicit_plan_selection_prefers_explicit_quality_over_original(self) -> None:
        candidate = self.registry.ingest(sample_candidate())

        payload = self.registry.plan_payload(candidate.object_id)

        self.assertIn("X-snsvideoflag=hd", payload["streams"][0]["url"])
        self.assertEqual(payload["streams"][0]["label"], "1080p · hd")
        self.assertEqual(payload["streams"][0]["wechatCapturedUrl"], "")

    def test_quality_selector_replaces_existing_case_insensitively(self) -> None:
        raw = sample_candidate()
        raw["media"][0]["urlToken"] = "&x-SnSvIdEoFlAg=sd&extra=two"
        candidate = self.registry.ingest(raw)
        spec_variant = next(
            variant for variant in candidate.variants.values() if variant.delivery_spec == "hd"
        )

        selected_url = urlsplit(spec_variant.url)
        selectors = [
            (name, value)
            for name, value in parse_qsl(selected_url.query)
            if name.lower() == "x-snsvideoflag"
        ]
        self.assertEqual(selectors, [("X-snsvideoflag", "hd")])
        self.assertIn("extra=two", selected_url.query)

    def test_original_variant_keeps_only_raw_encfilekey_and_token(self) -> None:
        payload = sample_candidate()
        payload["media"][0].update(
            {
                "url": (
                    "https://finder.video.qq.com/media/video.mp4?"
                    "encfilekey=encoded%2Bfile&hy=SZ&idx=1&m=transcoded"
                ),
                "urlToken": (
                    "&token=encoded%2Ftoken%3D&basedata=quality-bound"
                    "&sign=rotating-signature&web=1&svrnonce=123"
                ),
            }
        )
        candidate = self.registry.ingest(payload)
        original = next(
            variant for variant in candidate.variants.values() if not variant.delivery_spec
        )

        candidate_view = candidate.view()
        self.assertEqual(candidate_view["variants"][0]["deliverySpec"], "hd")
        self.assertTrue(candidate_view["variants"][-1]["isOriginal"])
        self.assertEqual(original.view()["quality"], "1080p · 原始视频")
        self.assertTrue(original.view()["isOriginal"])
        plan_stream = self.registry.plan_payload(
            candidate.object_id, original.variant_id
        )["streams"][0]
        selected_url = urlsplit(plan_stream["url"])
        self.assertEqual(
            selected_url.query,
            "encfilekey=encoded%2Bfile&token=encoded%2Ftoken%3D",
        )
        captured_url = urlsplit(plan_stream["wechatCapturedUrl"])
        self.assertEqual(
            captured_url.query,
            "encfilekey=encoded%2Bfile&hy=SZ&idx=1&m=transcoded&token=encoded%2Ftoken%3D"
            "&basedata=quality-bound&sign=rotating-signature&web=1&svrnonce=123",
        )
        self.assertNotIn("capturedUrl", original.view())
    ...
    def test_original_label_requires_feed_file_size(self) -> None:
        payload = sample_candidate()
        payload["media"][0].update(
            {
                "url": (
                ...
                "urlToken": "&token=access-token&sign=current-signature",
                "fileSize": 0,
                "specs": [],
            }
        )
        candidate = self.registry.ingest(payload)
        variant = next(iter(candidate.variants.values()))
        plan_stream = self.registry.plan_payload(
            candidate.object_id, variant.variant_id
        )["streams"][0]

        self.assertFalse(variant.view()["isOriginal"])
        self.assertEqual(plan_stream["wechatCapturedUrl"], "")
        self.assertIn("sign=current-signature", plan_stream["url"])
    ...
    def test_zero_key_is_clear_and_portrait_quality_uses_short_edge(self) -> None:
        ...
        candidate = self.registry.ingest(payload)
        view = candidate.view()
        self.assertEqual(view["variants"][0]["quality"], "1080p · hd")
        self.assertFalse(view["variants"][0]["isOriginal"])
...
(async function () {
  ...
  if (test.seenCount() > 64) process.exit(2);

  test.scan({ data: {
    recommendations: [feed("detail-related")],
    object: [feed("current-a")],
  ...
  const candidateIds = candidates.map(function (item) { return item.objectId; });
  // 推荐流与详情页的所有识别 feed 都上报候选
  for (let index = 0; index < 10; index += 1) {
    if (candidateIds.indexOf("preload-" + index) === -1) process.exit(3);
  }
  if (
    candidateIds.indexOf("current-a") === -1
    || candidateIds.indexOf("current-b") === -1
    || candidateIds.indexOf("detail-related") === -1
  ) process.exit(4);
...
class ProxyLeaseTests(unittest.TestCase):
    ...
    def test_pac_proxy_is_preserved_as_an_auto_proxy_configuration(self) -> None:
        snapshot = ProxySnapshot(
            {
                "ProxyEnable": RegistryValue(True, 0, 4),
                "ProxyServer": RegistryValue(False),
                ...
                "AutoDetect": RegistryValue(True, 1, 4),
            }
        )

        self.assertEqual(
            upstream_auto_proxy_config(snapshot),
            AutoProxyConfig(
                auto_config_url="http://127.0.0.1:10809/proxy.pac",
                auto_detect=True,
            ),
        )
...
class LoopbackProxyTests(unittest.TestCase):
    ...
    def test_plain_http_uses_the_per_url_pac_upstream(self) -> None:
        requested_urls: list[str] = []

        class Resolver:
            def resolve(self, target_url: str) -> tuple[str, int] | None:
                requested_urls.append(target_url)
                return upstream.server_address[:2]
        ...
        proxy = WechatLoopbackProxy(
            self.files,
            b"window.__session='__DOWNLOAD_STATION_SESSION__';",
            lambda _payload, _headers: None,
        ...
        try:
            ...
            client.sendall(
                b"GET http://vpn-only.example/resource HTTP/1.1\r\n"
                b"Host: vpn-only.example\r\nConnection: close\r\n\r\n"
            )
            response = self.read_all(client)
            client.close()
            self.assertIn(b"200 OK", response)
            self.assertIn(b"pac-upstream-ok", response)
            self.assertEqual(requested_urls, ["http://vpn-only.example/resource"])
        finally:
            proxy.stop()
            upstream.shutdown()
    ...
    def test_https_connect_uses_the_per_url_pac_upstream(self) -> None:
        resolver = Mock()
        resolver.resolve.return_value = ("127.0.0.1", 10809)
        remote = Mock()
        remote.recv.return_value = b"HTTP/1.1 200 Connection Established\r\n\r\n"
        self.proxy.replace_upstream(None, (), resolver)
        ...
        resolver.resolve.assert_called_once_with(
            "https://channels.weixin.qq.com:443/"
        )
        create_connection.assert_called_once_with(("127.0.0.1", 10809), timeout=15)
        self.assertIn(
