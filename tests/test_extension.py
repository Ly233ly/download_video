...
class ExtensionTests(unittest.TestCase):
    ...
    def test_manifests_are_versioned_and_include_structured_site_bridges(self) -> None:
        for name in ("manifest.json", "manifest.firefox.json"):
            ...
            self.assertEqual(manifest["version"], "1.4.9")
        ...
        self.assertIn('internal const string Version = "1.4.9"', setup)
        self.assertIn('AssemblyFileVersion("1.4.9.0")', setup)
        self.assertIn('AssemblyFileVersion("1.4.9.0")', launcher)
        version_resource = (ROOT / "packaging" / "download-transfer-station-version.txt").read_text(encoding="utf-8")
        self.assertIn("filevers=(1, 4, 9, 0)", version_resource)
        self.assertIn("prodvers=(1, 4, 9, 0)", version_resource)
