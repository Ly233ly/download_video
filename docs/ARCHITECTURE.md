...
- popup 只有“媒体 / 任务”一个应用根：媒体页投影候选组和创建前校验，任务页投影桌面计划状态；CSS/DOM 不拥有下载状态机。
- 顶层 HTTP(S) 地址变化统一进入页面身份生命周期：完整导航、History API 和片段跳转都会使旧异步识别失效，按用户清理设置处理旧候选，并通知内容脚本重新扫描当前 DOM。内容脚本另有低频地址核对作为 service worker 休眠后的保底。
- 独立窗口不绑定创建瞬间的标签页；service worker 只追踪最近活动的普通网页标签，扩展窗口焦点被忽略。普通标签切换或当前标签地址变化会通知独立窗口重新读取标题、地址、候选和发现状态。
...
- 主窗口启动只创建下载任务页；其他页面与设置子页按需创建，并在窗口空闲、非拖动阶段分批预热。页面事实仍来自原服务与数据库，不在界面层复制状态。
- 周期刷新只投影当前可见页。顶部状态使用轻量数据库汇总和后台探测的最近完成结果；隐藏页不枚举媒体候选、视频号候选或 IDM 行。
- 每个顶层窗口只有一个滚轮分发器，根据事件控件的父级关系选择最近滚动区。Treeview 和 Combobox 保留自己的滚动语义，卡片任意子控件都可驱动所属列表。
- Canvas 圆角控件复用已有图层，尺寸与状态事件在空闲周期合并；预览和缩略图按路径修订缓存并设置上限。窗口拖动期间暂停数据刷新，稳定后只执行一次布局和增量同步。
- 媒体列表查询保持只读；导入状态恢复发生在协调器启动恢复阶段。代理与视频号静态身份允许有界缓存，任务实际下载线路仍在每次任务开始时重新读取。
- Tk 线程只负责控件和对话框。诊断导出、批量历史清理、Eagle 来源更新、视频号启动/停止和预览读取都在后台执行，再通过有界 Queue 返回普通数据；结果携带代次，退出或新请求会使旧结果失效。
- 高频媒体变更只保留一个待处理信号，更新进度只保留最新数值，视频号预览由单工作线程只执行最新请求。所有 Queue 消费均有单轮上限，轮询 `after` ID 在退出时统一取消。
- 默认性能监测每 100ms 观察 Tk 心跳，只保留超过 50ms 的最近 256 条事件，并记录页面、当前操作、线程数和关键队列长度；诊断导出可携带这份有界快照，正常帧不写日志。
...
- 视频号捕获分为后台证书预检和后台启动/停止两个阶段；只有确认对话框在 Tk 主线程显示。桌面与扩展的批量任务清理调用认证服务边界并重新应用终态集合；单条清理按目标 ID 停止并移出计划/job。两种入口都保留本机文件、预览和 Eagle 内容。
...
以下数据流在 1.0.0 建立；图中的上游缓存/录制分支已由 1.2.0 章节取代。`eagle-bridge` 把默认可见候选、下载计划和本机合并接到原有持久队列。历史迁移材料可从 Git 历史恢复。
...
当前数据库 `user_version=6`；保留 `capture_sessions`、`media_groups`、`media_streams` 和 `download_plans`，删除旧 `component_files` 浏览器交接表。方案记录 `import_to_eagle`、`delete_after_import`、处理字节、预计字节、阶段说明和预览路径；schema 6 对旧计划的删除选择默认关闭。新请求明确 `importToEagle=1` 时默认建立清理选择，兼容覆盖安装后仍运行且尚未发送 `deleteAfterImport` 的旧扩展 worker；仅下载或显式关闭仍为 0。Eagle 导入成功后文件不再立即删除；处理器根据设置中的 `file_retention_days`（默认 7 天）延迟清理，到期后在 `process_once` 空闲时删除最终副本，成功后清空 `final_path`，失败则保留并记录错误。`/health` 同时公开产品版本、schema、扩展协议 1、`desktop_ffmpeg` 引擎与媒体工具就绪状态，供安装器的原子升级门使用。
...
- `/api/media/clear` 只批量删除终态计划；`/api/media/remove` 接受单个 `planId`，用于用户明确移出卡住或不再需要的目标计划。媒体计划、IDM job 与来源事件在同一事务内按外键安全顺序解除，随后清除没有计划引用的流、分组和捕获会话；任何入口都不删除最终文件、预览文件或 Eagle 项目。
- 扩展标签页候选维护单调递增的清理代次。清空当前媒体会立即清除候选、上下文和帧预览，并使清理前已经启动的清单读取、页面上下文或截图回调失效；清理后真实发生的新网络请求仍可进入当前代次。
- `ProcessingService` 把每日历史维护视为独立维护任务；清理异常不会退出主处理循环，后续下载、Eagle 导入和重试仍继续运行。
