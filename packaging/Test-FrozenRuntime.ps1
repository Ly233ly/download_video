...
try {
    ...
    if ([string]$health.version -ne "1.3.1" -or [int]$health.extensionProtocol -ne 1 -or [int]$health.databaseSchema -ne 6 -or -not [bool]$health.mediaReady -or -not [bool]$health.youtubeResolverReady -or [string]$health.downloadEngine -ne "desktop_ffmpeg" -or [string]$health.wechatChannels.state -ne "off" -or [string]$health.wechatChannels.bridgeHash -notmatch "^[0-9a-f]{16}$") {
        $healthSummary = $health | ConvertTo-Json -Compress
        throw "冻结后端健康门字段不符合 1.3.1 要求：$healthSummary"
    }
    ...
    if ([string]::IsNullOrWhiteSpace($EvidencePath)) {
        $EvidencePath = Join-Path $scratchRoot "frozen-runtime-1.3.1-evidence.json"
    }
