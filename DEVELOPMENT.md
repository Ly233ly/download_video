...
测试覆盖浏览器候选归组、质量选择、滚动位置、SPA/完整文档导航与冷启动清理顺序、认证与断线自动恢复、跨页面异步请求代次、卡住的响应头/正文超时、桌面下载、FFmpeg 合并、输出校验、系统代理、Eagle 可选降级、并发幂等补导、停止与完成竞态、重复内容状态、文件所有权、三重路径删除校验、后台线程异常恢复、健康探测 single-flight、大任务查询索引、缓存统计与安全清理、数据库迁移、Tk UI 键盘焦点、圆角/圆形抗锯齿像素、排他边界连续闭合、首次浅色与主题持久化、单次主题切换与响应式布局、微信视频号 feed、原始参数、VPN/PAC 上游、证书生命周期和退出恢复顺序。
