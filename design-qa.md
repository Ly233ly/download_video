...
# 1.4.7 微亮主题与全局圆角 Design QA

日期：2026-07-30

## 视觉证据与归一化

- 视觉真相：`C:\Users\Administrator\AppData\Local\Temp\codex-clipboard-34f71008-40f5-4ed5-bd32-4fc0753fcc30.png`，1477×986；用户标注了按钮、质量选择器、重复标题和主题方向。
- 微亮主题实现：`artifacts/ui-light-wechat-v2.png`，1216×839；逻辑客户区 1120×720，Windows 125% 显示缩放，截图包含原生标题栏。
- 深色主题实现：`artifacts/ui-dark-wechat-v2.png`，1216×839；逻辑客户区和显示密度同上。
- 同画布对照：`artifacts/ui-theme-comparison-final.png`，3199×720；三张截图按 720 px 高度等比归一化，依次为用户原图、微亮实现、深色实现。
- 状态差异：用户原图为“捕获关闭、无候选”，确定性夹具为“捕获中、有当前候选”；本轮只比较不受动态数据影响的壳层、按钮、标题、选择器、卡片和主题处理。

## 全视图与重点区域

- 全视图确认：应用内重复品牌标题已移除，Windows 原生标题显示“下载中转站 v1.4.7”；状态区仍保持右对齐，工作区获得更多垂直空间。
- 同画布中的捕获区、候选卡片和质量选择器已足够清楚地显示圆角、边框、颜色与间距，因此不再制作会重复同一证据的额外裁切。
- 微亮和深色截图使用同一逻辑视口与同一业务状态，适合直接检查双主题适配。

## 必查视觉面

- **字体与排版**：继续使用 Microsoft YaHei UI / Segoe UI 可用回退；标题、正文、辅助文字和按钮保持原有字号层级，主题切换没有造成换行、截断或字重跳变。
- **间距与布局节奏**：控件圆角令牌提升为缩略图 10、徽章 12、控件 14、卡片 18、大面板 22；按钮高度和原布局密度保持，视频候选改为带内边距的圆角卡片。
- **颜色与视觉令牌**：微亮主题使用低眩光灰紫背景、白色抬升表面和克制的靛紫主色；深色主题完整保留，成功、警告、危险、禁用与选中状态均有对应双主题色。
- **图像与资产**：继续复用项目已有产品图标与 PNG 操作图标；未引入 emoji、手绘 SVG、字符画或伪造图片。缩略图槽使用主题表面色，不再出现突兀的纯白方块。
- **文案与内容**：主题按钮明确显示下一步动作“切换到深色主题 / 切换到微亮主题”；标题版本来自 `APP_VERSION`，没有硬编码重复品牌。
- **交互与状态**：主题切换即时生效并写入数据库；质量选择器保留 `current/configure/cget` 选择契约，弹出列表使用主题表面；主按钮、危险按钮、次级按钮、分页和设置操作均改为可聚焦圆角控件。
- **响应与可访问性**：1120×720 下无重叠、裁切或额外水平滚动；按钮保留键盘焦点、Enter/Space 激活和禁用状态；状态不只依赖颜色，也保留文字。

## 对照迭代历史

1. 第一轮发现 P2：微亮主题候选行仍使用矩形 Frame/Separator，和提高后的全局圆角不一致；缩略图槽也出现纯白方块。修复为 18 px 圆角候选卡片、10 px 圆角缩略图槽，并让缩略图标签使用主题柔和表面。
2. 第一轮发现 P2：系统 Combobox 仍是直角原生控件，与圆角按钮和面板不匹配。替换为 14 px 圆角质量选择器与主题化弹出列表，保留原选择索引契约。
3. 复查证据：`artifacts/ui-light-wechat-v2.png`、`artifacts/ui-dark-wechat-v2.png` 和 `artifacts/ui-theme-comparison-final.png`。未发现遗留 P0/P1/P2。

## 验证

- Python 全量测试：239/239 通过。
- UI 定向测试：45/45 通过。
- 双主题真实 Tk 后台截图：通过。
- 运行时主题切换与持久化：通过。
- 1.4.7 UI 性能夹具：启动 353.80 ms，热刷新均值 5.28 ms，强制刷新均值 5.79 ms，60 次连续缩放 88.07 ms，待处理 Tk 回调 0。

P3 环境测试缺口：弹出质量列表和 Windows 标题栏在 150% / 200% 的物理显示器上仍建议随下一次安装包做一次人工观感复核；当前 125% 环境的绘制与交互契约已通过。
